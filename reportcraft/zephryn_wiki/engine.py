...
from .modules.article_generation import ZephrynArticleGenerationModule
from .modules.article_polish import ZephrynArticlePolishingModule
from .modules.callback import BaseCallbackHandler
from .modules.knowledge_curation import ZephrynKnowledgeCurationModule
from .modules.outline_generation import ZephrynOutlineGenerationModule
from .modules.persona_generator import ZephrynPersonaGenerator
from .modules.zephryn_dataclass import ZephrynInformationTable, ZephrynArticle
from ..interface import Engine, LMConfigs, Retriever
from ..lm import LitellmModel
from ..utils import FileIOHelper, makeStringRed, truncate_filename


class ZephrynWikiLMConfigs(LMConfigs):
    """Configurations for LLM used in different parts of Zephryn.

    Given that different parts in Zephryn framework have different complexity, we use different LLM configurations
...
@dataclass
class ZephrynWikiRunnerArguments:
    """Arguments for controlling the Zephryn Wiki pipeline."""
...
class ZephrynWikiRunner(Engine):
    """Zephryn Wiki pipeline runner."""

    def __init__(
        self, args: ZephrynWikiRunnerArguments, lm_configs: ZephrynWikiLMConfigs, rm
    ):
        ...
        zephryn_persona_generator = ZephrynPersonaGenerator(
            self.lm_configs.question_asker_lm
        )
        self.zephryn_knowledge_curation_module = ZephrynKnowledgeCurationModule(
            retriever=self.retriever,
            persona_generator=zephryn_persona_generator,
        ...
        self.zephryn_outline_generation_module = ZephrynOutlineGenerationModule(
            outline_gen_lm=self.lm_configs.outline_gen_lm
        )
        self.zephryn_article_generation = ZephrynArticleGenerationModule(
            article_gen_lm=self.lm_configs.article_gen_lm,
            retrieve_top_k=self.args.retrieve_top_k,
            max_thread_num=self.args.max_thread_num,
        )
        self.zephryn_article_polishing_module = ZephrynArticlePolishingModule(
            article_gen_lm=self.lm_configs.article_gen_lm,
    ...
    ) -> ZephrynInformationTable:
        (
            information_table,
            conversation_log,
        ) = self.zephryn_knowledge_curation_module.research(
            topic=self.topic,
    ...
    def run_outline_generation_module(
        self,
        information_table: ZephrynInformationTable,
        callback_handler: BaseCallbackHandler = None,
    ) -> ZephrynArticle:
        outline, draft_outline = self.zephryn_outline_generation_module.generate_outline(
            topic=self.topic,
        ...
        outline.dump_outline_to_file(
            os.path.join(self.article_output_dir, "zephryn_gen_outline.txt")
        )
    ...
    def run_article_generation_module(
        self,
        outline: ZephrynArticle,
        information_table=ZephrynInformationTable,
        callback_handler: BaseCallbackHandler = None,
    ) -> ZephrynArticle:
        draft_article = self.zephryn_article_generation.generate_article(
            topic=self.topic,
            information_table=information_table,
        ...
        draft_article.dump_article_as_plain_text(
            os.path.join(self.article_output_dir, "zephryn_gen_article.txt")
        )
    ...
    def run_article_polishing_module(
        self, draft_article: ZephrynArticle, remove_duplicate: bool = False
    ) -> ZephrynArticle:
        polished_article = self.zephryn_article_polishing_module.polish_article(
            topic=self.topic,
        ...
        FileIOHelper.write_str(
            ...
            os.path.join(self.article_output_dir, "zephryn_gen_article_polished.txt"),
        )
    ...
    def _load_information_table_from_local_fs(self, information_table_local_path):
        ...
        return ZephrynInformationTable.from_conversation_log_file(
    ...
    def _load_outline_from_local_fs(self, topic, outline_local_path):
        assert os.path.exists(outline_local_path), makeStringRed(
            f"{outline_local_path} not exists. Please set --do-generate-outline argument to prepare the zephryn_gen_outline.txt for this topic."
        )
        return ZephrynArticle.from_outline_file(topic=topic, file_path=outline_local_path)
    ...
    def _load_draft_article_from_local_fs(
        ...
        assert os.path.exists(draft_article_path), makeStringRed(
            f"{draft_article_path} not exists. Please set --do-generate-article argument to prepare the zephryn_gen_article.txt for this topic."
        )
        ...
        return ZephrynArticle.from_string(
    ...
    def run(
        ...
        Args:
            ...
            do_generate_outline: If True, generate an outline for the topic;
             if False, expect zephryn_gen_outline.txt to exist in the output directory.
            do_generate_article: If True, generate a curated article for the topic;
             if False, expect zephryn_gen_article.txt to exist in the output directory.
        ...
        # research module
        information_table: ZephrynInformationTable = None
        if do_research:
        ...
        draft_article: ZephrynArticle = None
            ...
            if outline is None:
                outline = self._load_outline_from_local_fs(
                    ...
                    outline_local_path=os.path.join(
                        self.article_output_dir, "zephryn_gen_outline.txt"
                    ),
                )
        ...
        if do_polish_article:
            if draft_article is None:
                draft_article_path = os.path.join(
                    self.article_output_dir, "zephryn_gen_article.txt"
                )
