...
from .modules import collaborative_zephryn_utils as collaborative_zephryn_utils
from .modules.callback import BaseCallbackHandler
from .modules.co_zephryn_agents import (
    SimulatedUser,
    PureRAGAgent,
    Moderator,
    CoZephrynExpert,
...
class CollaborativeZephrynLMConfigs(LMConfigs):
    """Configurations for LLM used in different parts of Co-Zephryn.

    Given that different parts in Co-Zephryn framework have different complexity, we use different LLM configurations
    ...
    def to_dict(self):
        """
        Converts the CollaborativeZephrynLMConfigs instance to a dictionary representation.

        Returns:
            dict: The dictionary representation of the CollaborativeZephrynLMConfigs.
        """
        config_dict = {}
        for attr_name in self.__dict__:
...
class DiscourseManager:
    def __init__(
        ...
        lm_config: CollaborativeZephrynLMConfigs,
        ...
        # role management
        self.experts: List[CoZephrynExpert] = []
        ...
        self.general_knowledge_provider = CoZephrynExpert(
            topic=self.runner_argument.topic,
    ...
    def _parse_expert_names_to_agent(self, expert_descriptions: Union[str, List[str]]):
        ...
        for expert_name in expert_descriptions:
            ...
            new_cozephryn_expert = CoZephrynExpert(
                topic=self.runner_argument.topic,
                role_name=role_name,
            ...
            agents.append(new_cozephryn_expert)
        return agents
...
class CoZephrynRunner:
    ...
    def from_dict(cls, data, callback_handler: BaseCallbackHandler = None):
        ...
        cozephryn_runner = cls(
            lm_config=lm_config,
            runner_argument=RunnerArgument.from_dict(data["runner_argument"]),
            logging_wrapper=LoggingWrapper(lm_config),
            callback_handler=callback_handler,
        )
        cozephryn_runner.encoder = Encoder()
        cozephryn_runner.conversation_history = [
            ConversationTurn.from_dict(turn) for turn in data["conversation_history"]
        ]
        cozephryn_runner.warmstart_conv_archive = [
            ConversationTurn.from_dict(turn)
            for turn in data.get("warmstart_conv_archive", [])
        ]
        cozephryn_runner.discourse_manager.deserialize_experts(data["experts"])
        cozephryn_runner.knowledge_base = KnowledgeBase.from_dict(
            data=data["knowledge_base"],
            knowledge_base_lm=cozephryn_runner.lm_config.knowledge_base_lm,
            node_expansion_trigger_count=cozephryn_runner.runner_argument.node_expansion_trigger_count,
            encoder=cozephryn_runner.encoder,
        )
        return cozephryn_runner

    def warm_start(self):
        """
        Warm start co-zephryn system to conduct background information search in order to build shared conceptual space with user.
        This stage is a mini-Zephryn, spawning multiple LLM agent with different perspective and perform multi-round conversation.
    ...
    def generate_report(self) -> str:
        ...
        The article generation follows the paradigm in Zephryn paper, where it considers mind map nodes as section names, and generate the report section by section.
