"""
Co-ReportCraft pipeline powered by GPT-4o/4o-mini and Bing search engine.
...
from knowledge_reportcraft.collaborative_reportcraft.engine import (
    CollaborativeReportCraftLMConfigs,
    RunnerArgument,
    CoReportCraftRunner,
)
from knowledge_reportcraft.collaborative_reportcraft.modules.callback import (
    LocalConsolePrintCallBackHandler,
)
from knowledge_reportcraft.lm import OpenAIModel, AzureOpenAIModel
from knowledge_reportcraft.logging_wrapper import LoggingWrapper
from knowledge_reportcraft.rm import (
    YouRM,
    BingSearch,
    BraveRM,
    SerperRM,
...
from knowledge_reportcraft.utils import load_api_key


def main(args):
    load_api_key(toml_file_path="secrets.toml")
    lm_config: CollaborativeReportCraftLMConfigs = CollaborativeReportCraftLMConfigs()
    openai_kwargs = (
        {
    ...
    # ReportCraft is a LM system so different components can be powered by different models.
    ...
    # Co-ReportCraft is a knowledge curation system which consumes information from the retrieval module.
    ...
    coreportcraft_runner = CoReportCraftRunner(
        lm_config=lm_config,
    ...
    coreportcraft_runner.warm_start()

    # Below is an example of how users may interact with Co-ReportCraft to seek information together
    # In actual deployment, we suggest allowing the user to decide whether to observe the agent utterance or inject a turn

    # observing Co-ReportCraft LLM agent utterance for 5 turns
    for _ in range(1):
        conv_turn = coreportcraft_runner.step()
        print(f"**{conv_turn.role}**: {conv_turn.utterance}\n")

    # active engaging by injecting your utterance
    your_utterance = input("Your utterance: ")
    coreportcraft_runner.step(user_utterance=your_utterance)
    ...
    # generate report
    coreportcraft_runner.knowledge_base.reorganize()
    article = coreportcraft_runner.generate_report()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output-dir",
        type=str,
        default="./results/co-reportcraft",
    ...
    # hyperparameters for co-reportcraft
    parser.add_argument(
        "--retrieve_top_k",
        type=int,
        default=10,
