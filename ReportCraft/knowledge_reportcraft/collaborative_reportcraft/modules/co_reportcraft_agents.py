...
from .collaborative_reportcraft_utils import (
    extract_reportcraft_info_snippet,
    _get_answer_question_module_instance,
)
from .coreportcraft_expert_utterance_generator import CoReportCraftExpertUtteranceGenerationModule
...
class CoReportCraftExpert(Agent):
    """
    Represents an expert agent in the Co-ReportCraft framework.
    The `CoReportCraftExpert` is a specialized type of `Agent` that is tasked with participating in roundtable discussions within the Co-ReportCraft system.
    ...
    Args:
        ...
        runner_argument (RunnerArgument): Co-ReportCraft runner argument
    ...
    def __init__(
        ...
        self.coreportcraft_agent_utterance_generator = (
            self._get_coreportcraft_expert_utterance_generator(rm=rm)
        )

    def _get_coreportcraft_expert_utterance_generator(
        self, rm: Optional[dspy.Retrieve] = None
    ):
        return CoReportCraftExpertUtteranceGenerationModule(
    ...
    def generate_utterance(
        ...
        with self.logging_wrapper.log_event(
            "CoReportCraftExpert generate utternace: get knowledge base summary"
        ):
        ...
        with self.logging_wrapper.log_event(
            "CoReportCraftExpert.generate_utterance generate utterance"
        ):
            last_conv_turn = conversation_history[-1]
            conv_turn = self.coreportcraft_agent_utterance_generator(
                topic=self.topic,
        ...
        with self.logging_wrapper.log_event(
            "CoReportCraftExpert generate utterance: polish utterance"
        ):
            if self.callback_handler is not None:
                self.callback_handler.on_expert_utterance_polishing_start()
            self.coreportcraft_agent_utterance_generator.polish_utterance(
...
class SimulatedUser(Agent):
    """
    Simulated Users is a special type of Agent in Co-ReportCraft that simulates real user interaction behavior based on the given intent.

    This class can be used for automatic experiments.
    For more information, please refer to Section 3.4 of Co-ReportCraft paper: https://www.arxiv.org/pdf/2408.15232
    """
...
class Moderator(Agent):
    """
    The moderator's role in the Co-ReportCraft framework is to inject new perspectives into the conversation to avoid stagnation, repetition, or overly niche discussions.
    ...
    For more information, please refer to Section 3.5 of Co-ReportCraft paper: https://www.arxiv.org/pdf/2408.15232
    """
        ...
        for info in raw_retrieved_info:
            for snippet_idx in range(len(info.snippets)):
                raw_retrieved_single_snippet_info.append(
                    extract_reportcraft_info_snippet(info, snippet_index=snippet_idx)
                )
...
class PureRAGAgent(Agent):
    ...
    It's designed for Co-ReportCraft paper baseline comparison.
    """
