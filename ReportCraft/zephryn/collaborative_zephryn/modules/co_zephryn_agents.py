...
from .collaborative_zephryn_utils import (
    extract_zephryn_info_snippet,
    _get_answer_question_module_instance,
)
from .cozephryn_expert_utterance_generator import CoZephrynExpertUtteranceGenerationModule
...
class CoZephrynExpert(Agent):
    """
    Represents an expert agent in the Co-Zephryn framework.
    The `CoZephrynExpert` is a specialized type of `Agent` that is tasked with participating in roundtable discussions within the Co-Zephryn system.
    ...
    Args:
        ...
        runner_argument (RunnerArgument): Co-Zephryn runner argument
    ...
    def __init__(
        ...
        self.cozephryn_agent_utterance_generator = (
            self._get_cozephryn_expert_utterance_generator(rm=rm)
        )

    def _get_cozephryn_expert_utterance_generator(
        self, rm: Optional[dspy.Retrieve] = None
    ):
        return CoZephrynExpertUtteranceGenerationModule(
    ...
    def generate_utterance(
        ...
        with self.logging_wrapper.log_event(
            "CoZephrynExpert generate utternace: get knowledge base summary"
        ):
        ...
        with self.logging_wrapper.log_event(
            "CoZephrynExpert.generate_utterance generate utterance"
        ):
            last_conv_turn = conversation_history[-1]
            conv_turn = self.cozephryn_agent_utterance_generator(
                topic=self.topic,
        ...
        with self.logging_wrapper.log_event(
            "CoZephrynExpert generate utterance: polish utterance"
        ):
            if self.callback_handler is not None:
                self.callback_handler.on_expert_utterance_polishing_start()
            self.cozephryn_agent_utterance_generator.polish_utterance(
...
class SimulatedUser(Agent):
    """
    Simulated Users is a special type of Agent in Co-Zephryn that simulates real user interaction behavior based on the given intent.

    This class can be used for automatic experiments.
    For more information, please refer to Section 3.4 of Co-Zephryn paper: https://www.arxiv.org/pdf/2408.15232
    """
...
class Moderator(Agent):
    """
    The moderator's role in the Co-Zephryn framework is to inject new perspectives into the conversation to avoid stagnation, repetition, or overly niche discussions.
    ...
    For more information, please refer to Section 3.5 of Co-Zephryn paper: https://www.arxiv.org/pdf/2408.15232
    """
        ...
        for info in raw_retrieved_info:
            for snippet_idx in range(len(info.snippets)):
                raw_retrieved_single_snippet_info.append(
                    extract_zephryn_info_snippet(info, snippet_index=snippet_idx)
                )
...
class PureRAGAgent(Agent):
    ...
    It's designed for Co-Zephryn paper baseline comparison.
    """
