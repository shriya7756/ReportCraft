...
def extract_zephryn_info_snippet(info: Information, snippet_index: int) -> Information:
    """
    Constructs a new Information instance with only the specified snippet index.

    Args:
        zephryn_info (Information): The original Information instance.
        ...
        for info in searched_results:
            if i < len(info.snippets) and not abort:
                ...
                if cur_snippet not in included_snippets:
                    ...
                    info = extract_zephryn_info_snippet(info, snippet_index=i)
...
def extract_cited_zephryn_info(
    response: str, index_to_zephryn_info: Dict[int, Information]
    ...
    Args:
        ...
        index_to_zephryn_info (Dict[int, Information]): A dictionary mapping indices to Information instances.
    ...
    cited_zephryn_info = {
        index: info
        for index, info in index_to_zephryn_info.items()
        if index in cited_indices
    }
    return cited_zephryn_info
