...
# If you install the source code instead of the `knowledge-reportcraft` package,
...
from knowledge_reportcraft.lm import OpenAIModel
from knowledge_reportcraft.rm import YouRM
from knowledge_reportcraft.reportcraft_wiki.modules.callback import BaseCallbackHandler
from knowledge_reportcraft.utils import truncate_filename
...
class DemoFileIOHelper:
    ...
    def assemble_article_data(article_file_path_dict):
        ...
        """
        if (
            "reportcraft_gen_article.txt" in article_file_path_dict
            or "reportcraft_gen_article_polished.txt" in article_file_path_dict
        ):
            full_article_name = (
                "reportcraft_gen_article_polished.txt"
                if "reportcraft_gen_article_polished.txt" in article_file_path_dict
                else "reportcraft_gen_article.txt"
            )
            article_data = {
...
def _display_main_article(
    ...
    if show_conversation and "conversation_log" in article_data:
        with st.expander(
            "**ReportCraft** is powered by a knowledge agent that proactively researches a given topic by asking good questions from different perspectives.\n\n"
            ":sunglasses: Click here to view the agent's brainreportcrafting process!"
        ):
...
def set_reportcraft_runner():
    current_working_dir = os.path.join(get_demo_dir(), "DEMO_WORKING_DIR")
    if not os.path.exists(current_working_dir):
        os.makedirs(current_working_dir)

    # configure ReportCraft runner
    llm_configs = ReportCraftWikiLMConfigs()
    ...
    runner = ReportCraftWikiRunner(engine_args, llm_configs, rm)
