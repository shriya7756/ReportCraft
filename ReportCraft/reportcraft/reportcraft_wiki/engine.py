...
from .modules.article_generation import ReportCraftArticleGenerationModule
from .modules.article_polish import ReportCraftArticlePolishingModule
from .modules.callback import BaseCallbackHandler
from .modules.knowledge_curation import ReportCraftKnowledgeCurationModule
from .modules.outline_generation import ReportCraftOutlineGenerationModule
from .modules.persona_generator import ReportCraftPersonaGenerator
from .modules.reportcraft_dataclass import ReportCraftInformationTable, ReportCraftArticle
from ..interface import Engine, LMConfigs, Retriever
from ..lm import LitellmModel
from ..utils import FileIOHelper, makeStringRed, truncate_filename


class ReportCraftWikiLMConfigs(LMConfigs):
    """Configurations for LLM used in different parts of ReportCraft.

    Given that different parts in ReportCraft framework have different complexity, we use different LLM configurations
...
@dataclass
class ReportCraftWikiRunnerArguments:
    """Arguments for controlling the ReportCraft Wiki pipeline."""
...
class ReportCraftWikiRunner(Engine):
    """ReportCraft Wiki pipeline runner."""

    def __init__(
        self, args: ReportCraftWikiRunnerArguments, lm_configs: ReportCraftWikiLMConfigs, rm
    ):
        ...
        reportcraft_persona_generator = ReportCraftPersonaGenerator(
            self.lm_configs.question_asker_lm
        )
        self.reportcraft_knowledge_curation_module = ReportCraftKnowledgeCurationModule(
            retriever=self.retriever,
            persona_generator=reportcraft_persona_generator,
        ...
        self.reportcraft_outline_generation_module = ReportCraftOutlineGenerationModule(
            outline_gen_lm=self.lm_configs.outline_gen_lm
        )
        self.reportcraft_article_generation = ReportCraftArticleGenerationModule(
            article_gen_lm=self.lm_configs.article_gen_lm,
            retrieve_top_k=self.args.retrieve_top_k,
            max_thread_num=self.args.max_thread_num,
        )
        self.reportcraft_article_polishing_module = ReportCraftArticlePolishingModule(
            article_gen_lm=self.lm_configs.article_gen_lm,
    ...
    ) -> ReportCraftInformationTable:
        (
            information_table,
            conversation_log,
        ) = self.reportcraft_knowledge_curation_module.research(
            topic=self.topic,
    ...
    def run_outline_generation_module(
        self,
        information_table: ReportCraftInformationTable,
        callback_handler: BaseCallbackHandler = None,
    ) -> ReportCraftArticle:
        outline, draft_outline = self.reportcraft_outline_generation_module.generate_outline(
            topic=self.topic,
        ...
        outline.dump_outline_to_file(
            os.path.join(self.article_output_dir, "reportcraft_gen_outline.txt")
        )
    ...
    def run_article_generation_module(
        self,
        outline: ReportCraftArticle,
        information_table=ReportCraftInformationTable,
        callback_handler: BaseCallbackHandler = None,
    ) -> ReportCraftArticle:
        draft_article = self.reportcraft_article_generation.generate_article(
            topic=self.topic,
            information_table=information_table,
        ...
        draft_article.dump_article_as_plain_text(
            os.path.join(self.article_output_dir, "reportcraft_gen_article.txt")
        )
    ...
    def run_article_polishing_module(
        self, draft_article: ReportCraftArticle, remove_duplicate: bool = False
    ) -> ReportCraftArticle:
        polished_article = self.reportcraft_article_polishing_module.polish_article(
            topic=self.topic,
        ...
        FileIOHelper.write_str(
            ...
            os.path.join(self.article_output_dir, "reportcraft_gen_article_polished.txt"),
        )
    ...
    def _load_information_table_from_local_fs(self, information_table_local_path):
        ...
        return ReportCraftInformationTable.from_conversation_log_file(
    ...
    def _load_outline_from_local_fs(self, topic, outline_local_path):
        assert os.path.exists(outline_local_path), makeStringRed(
            f"{outline_local_path} not exists. Please set --do-generate-outline argument to prepare the reportcraft_gen_outline.txt for this topic."
        )
        return ReportCraftArticle.from_outline_file(topic=topic, file_path=outline_local_path)
    ...
    def _load_draft_article_from_local_fs(
        ...
        assert os.path.exists(draft_article_path), makeStringRed(
            f"{draft_article_path} not exists. Please set --do-generate-article argument to prepare the reportcraft_gen_article.txt for this topic."
        )
        ...
        return ReportCraftArticle.from_string(
    ...
    def run(
        ...
        Args:
            ...
            do_generate_outline: If True, generate an outline for the topic;
             if False, expect reportcraft_gen_outline.txt to exist in the output directory.
            do_generate_article: If True, generate a curated article for the topic;
             if False, expect reportcraft_gen_article.txt to exist in the output directory.
        ...
        # research module
        information_table: ReportCraftInformationTable = None
        if do_research:
        ...
        outline: ReportCraftArticle = None
        ...
        draft_article: ReportCraftArticle = None
            ...
            if outline is None:
                outline = self._load_outline_from_local_fs(
                    ...
                    outline_local_path=os.path.join(
                        self.article_output_dir, "reportcraft_gen_outline.txt"
                    ),
                )
        ...
        if do_polish_article:
            if draft_article is None:
                draft_article_path = os.path.join(
                    self.article_output_dir, "reportcraft_gen_article.txt"
                )
