...
class ReportCraftInformationTable(InformationTable):
    """
    ...
    in ReportCraft paper https://arxiv.org/pdf/2402.14207.pdf, additional information
    ...
    def __init__(self, conversations=List[Tuple[str, List[DialogueTurn]]]):
        ...
        self.url_to_info: Dict[str, Information] = (
            ReportCraftInformationTable.construct_url_to_info(self.conversations)
        )
    ...
    def construct_url_to_info(
        ...
        for persona, conv in conversations:
            for turn in conv:
                for reportcraft_info in turn.search_results:
                    if reportcraft_info.url in url_to_info:
                        url_to_info[reportcraft_info.url].snippets.extend(reportcraft_info.snippets)
                    else:
                        url_to_info[reportcraft_info.url] = reportcraft_info
...
class ReportCraftArticle(Article):
    ...
    def _merge_new_info_to_references(
        ...
        """
        Merges new reportcraft information into existing references and updates the citation index mapping.

        Args:
        new_info_list (List[Information]): A list of dictionaries representing new reportcraft information.
        index_to_keep (List[int]): A list of index of the new_info_list to keep. If none, keep all.

        Returns:
        Dict[int, int]: A dictionary mapping the index of each reportcraft information piece in the input list
                        to its unified citation index in the references.
        """
        citation_idx_mapping = {}
        for idx, reportcraft_info in enumerate(new_info_list):
            if index_to_keep is not None and idx not in index_to_keep:
                continue
            url = reportcraft_info.url
            if url not in self.reference["url_to_unified_index"]:
                ...
                self.reference["url_to_info"][url] = reportcraft_info
            else:
                existing_snippets = self.reference["url_to_info"][url].snippets
                existing_snippets.extend(reportcraft_info.snippets)
    ...
    def from_outline_file(cls, topic: str, file_path: str):
        """
        Create ReportCraftArticle class instance from outline file.
        """
        outline_str = FileIOHelper.load_str(file_path)
        return ReportCraftArticle.from_outline_str(topic=topic, outline_str=outline_str)

    @classmethod
    def from_outline_str(cls, topic: str, outline_str: str):
        """
        Create ReportCraftArticle class instance from outline only string.
        """
        lines = []
        try:
