...
def extract_reportcraft_info_snippet(info: Information, snippet_index: int) -> Information:
    """
    Constructs a new Information instance with only the specified snippet index.

    Args:
        reportcraft_info (Information): The original Information instance.
    ...
    new_reportcraft_info = Information(
        info.url, info.description, new_snippets, info.title, info.meta
    )
    return new_reportcraft_info
        ...
        for info in searched_results:
            if i < len(info.snippets) and not abort:
                ...
                if cur_snippet not in included_snippets:
                    ...
                    info = extract_reportcraft_info_snippet(info, snippet_index=i)
...
def extract_cited_reportcraft_info(
    response: str, index_to_reportcraft_info: Dict[int, Information]
    ...
    Args:
        ...
        index_to_reportcraft_info (Dict[int, Information]): A dictionary mapping indices to Information instances.
    ...
    cited_reportcraft_info = {
        index: info
        for index, info in index_to_reportcraft_info.items()
        if index in cited_indices
    }
    return cited_reportcraft_info
