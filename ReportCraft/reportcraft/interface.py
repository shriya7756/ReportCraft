...
class Information:
    ...
    it with a description, snippets, and title of the reportcraft information.
    ...
    def from_dict(cls, info_dict):
        """Create a Information object from a dictionary.
           Usage: info = Information.from_dict(reportcraft_info_dict)
        ...
        def process_query(q):
            ...
            for data in retrieved_data_list:
                for i in range(len(data["snippets"])):
                    # ReportCraft generate the article with citations. We do not consider multi-hop citations.
                    # Remove citations in the source to avoid confusion.
                    data["snippets"][i] = ArticleTextProcessing.remove_citations(
                        data["snippets"][i]
                    )
                reportcraft_info = Information.from_dict(data)
                reportcraft_info.meta["query"] = q
                local_to_return.append(reportcraft_info)
...
class Agent(ABC):
    """
    Interface for ReportCraft and Co-ReportCraft LLM agent
    ...
    Args:
        knowledge_base (KnowledgeBase): The current knowledge base (e.g., mind map in Co-ReportCraft) that contains the accumulated information relevant to the conversation.
