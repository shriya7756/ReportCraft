# Contributing to ReportCraft

Thank you for your interest in contributing to ReportCraft!

Contributions aren't just about code. We are accepting the following forms of contribution:
- Pull requests for additional language model support to `knowledge_reportcraft/lm.py`.
- Pull requests for additional retrieval model/search engine support to `knowledge_reportcraft/rm.py`.
...
- Please clearly describe the required API keys and provide instructions on how to get them (if applicable). This project manages API keys with `secrets.toml`.

**Code Format:**

We adopt [`black`](https://github.com/psf/black) for arranging and formatting Python code. To streamline the contribution process, we set up a [pre-commit hook](https://pre-commit.com/) to format the code under `knowledge_reportcraft/` before committing. To install the pre-commit hook, run:
```
