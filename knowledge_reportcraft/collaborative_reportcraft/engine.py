...
from .modules import collaborative_reportcraft_utils as collaborative_reportcraft_utils
from .modules.callback import BaseCallbackHandler
from .modules.co_reportcraft_agents import (
    SimulatedUser,
    PureRAGAgent,
    Moderator,
    CoReportCraftExpert,
...
class CollaborativeReportCraftLMConfigs(LMConfigs):
    """Configurations for LLM used in different parts of Co-ReportCraft.

    Given that different parts in Co-ReportCraft framework have different complexity, we use different LLM configurations
    ...
    def to_dict(self):
        """
        Converts the CollaborativeReportCraftLMConfigs instance to a dictionary representation.

        Returns:
            dict: The dictionary representation of the CollaborativeReportCraftLMConfigs.
        """
        config_dict = {}
        for attr_name in self.__dict__:
...
class DiscourseManager:
    def __init__(
        ...
        lm_config: CollaborativeReportCraftLMConfigs,
        ...
        # role management
        self.experts: List[CoReportCraftExpert] = []
        ...
        self.general_knowledge_provider = CoReportCraftExpert(
            topic=self.runner_argument.topic,
    ...
    def _parse_expert_names_to_agent(self, expert_descriptions: Union[str, List[str]]):
        ...
        for expert_name in expert_descriptions:
            ...
            new_coreportcraft_expert = CoReportCraftExpert(
                topic=self.runner_argument.topic,
                role_name=role_name,
            ...
            agents.append(new_coreportcraft_expert)
        return agents
...
class CoReportCraftRunner:
    ...
    def from_dict(cls, data, callback_handler: BaseCallbackHandler = None):
        ...
        coreportcraft_runner = cls(
            lm_config=lm_config,
            runner_argument=RunnerArgument.from_dict(data["runner_argument"]),
            logging_wrapper=LoggingWrapper(lm_config),
            callback_handler=callback_handler,
        )
        coreportcraft_runner.encoder = Encoder()
        coreportcraft_runner.conversation_history = [
            ConversationTurn.from_dict(turn) for turn in data["conversation_history"]
        ]
        coreportcraft_runner.warmstart_conv_archive = [
            ConversationTurn.from_dict(turn)
            for turn in data.get("warmstart_conv_archive", [])
        ]
        coreportcraft_runner.discourse_manager.deserialize_experts(data["experts"])
        coreportcraft_runner.knowledge_base = KnowledgeBase.from_dict(
            data=data["knowledge_base"],
            knowledge_base_lm=coreportcraft_runner.lm_config.knowledge_base_lm,
            node_expansion_trigger_count=coreportcraft_runner.runner_argument.node_expansion_trigger_count,
            encoder=coreportcraft_runner.encoder,
        )
        return coreportcraft_runner

    def warm_start(self):
        """
        Warm start co-reportcraft system to conduct background information search in order to build shared conceptual space with user.
        This stage is a mini-ReportCraft, spawning multiple LLM agent with different perspective and perform multi-round conversation.
    ...
    def generate_report(self) -> str:
        ...
        The article generation follows the paradigm in ReportCraft paper, where it considers mind map nodes as section names, and generate the report section by section.
