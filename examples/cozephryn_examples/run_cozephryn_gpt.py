"""
Co-Zephryn pipeline powered by GPT-4o/4o-mini and Bing search engine.
...
from knowledge_zephryn.collaborative_zephryn.engine import (
    CollaborativeZephrynLMConfigs,
    RunnerArgument,
    CoZephrynRunner,
)
from knowledge_zephryn.collaborative_zephryn.modules.callback import (
    LocalConsolePrintCallBackHandler,
)
from knowledge_zephryn.lm import OpenAIModel, AzureOpenAIModel
from knowledge_zephryn.logging_wrapper import LoggingWrapper
from knowledge_zephryn.rm import (
    YouRM,
    BingSearch,
    BraveRM,
    SerperRM,
...
from knowledge_zephryn.utils import load_api_key


def main(args):
    load_api_key(toml_file_path="secrets.toml")
    lm_config: CollaborativeZephrynLMConfigs = CollaborativeZephrynLMConfigs()
    openai_kwargs = (
        {
    ...
    # Zephryn is a LM system so different components can be powered by different models.
    ...
    # Co-Zephryn is a knowledge curation system which consumes information from the retrieval module.
    ...
    cozephryn_runner = CoZephrynRunner(
        lm_config=lm_config,
    ...
    cozephryn_runner.warm_start()

    # Below is an example of how users may interact with Co-Zephryn to seek information together
    # In actual deployment, we suggest allowing the user to decide whether to observe the agent utterance or inject a turn

    # observing Co-Zephryn LLM agent utterance for 5 turns
    for _ in range(1):
        conv_turn = cozephryn_runner.step()
        print(f"**{conv_turn.role}**: {conv_turn.utterance}\n")

    # active engaging by injecting your utterance
    your_utterance = input("Your utterance: ")
    cozephryn_runner.step(user_utterance=your_utterance)
    ...
    # generate report
    cozephryn_runner.knowledge_base.reorganize()
    article = cozephryn_runner.generate_report()
...
if __name__ == "__main__":
    ...
    # hyperparameters for co-zephryn
    parser.add_argument(
        "--retrieve_top_k",
        type=int,
        default=10,
