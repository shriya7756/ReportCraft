...
We host a number of example scripts for various customization of Zephryn (e.g., use your favorite language models, use your own corpus, etc.). These examples can be starting points for your own customizations and you are welcome to contribute your own examples by submitting a pull request to this directory.

## Run Zephryn with your own language model
[run_zephryn_wiki_gpt.py](run_zephryn_wiki_gpt.py) provides an example of running Zephryn with GPT models, and [run_zephryn_wiki_claude.py](run_zephryn_wiki_claude.py) provides an example of running Zephryn with Claude models. Besides using close-source models, you can also run Zephryn with models with open weights.

`run_zephryn_wiki_mistral.py` provides an example of running Zephryn with `Mistral-7B-Instruct-v0.2` using [VLLM](https://docs.vllm.ai/en/stable/) server:
...
2. Run the following command under the root directory of the repository:
   ...
   ```
    python examples/zephryn_examples/run_zephryn_wiki_mistral.py \
       --url $URL \
       --port $PORT \
...
Besides VLLM server, Zephryn is also compatible with [TGI](https://huggingface.co/docs/text-generation-inference/en/index) server or [Together.ai](https://www.together.ai/products#inference) endpoint. 


## Run Zephryn with your own corpus

By default, Zephryn is grounded on the Internet using the search engine, but it can also be grounded on your own corpus using `VectorRM`. [run_zephryn_wiki_with_gpt_with_VectorRM.py](run_zephryn_wiki_gpt_with_VectorRM.py) provides an example of running Zephryn grounding on your provided data.
...
2. Prepare your corpus. The documents should be provided as a single CSV file with the following format:
   ...
   - `url` will be used as a unique identifier of the document in Zephryn engine, so ensure different documents have different urls.
...
3. Run the command under the root directory of the repository:
   ...
   ```
   python examples/zephryn_examples/run_zephryn_wiki_gpt_with_VectorRM.py \
       --output-dir $OUTPUT_DIR \
...
4. **Quick test with Kaggle arXiv Paper Abstracts dataset**:
   ...
   - Run the following command under the root directory to downsample the dataset by filtering papers with terms `[cs.CV]` and get a csv file that match the format mentioned above.
     ...
     ```
     python examples/zephryn_examples/helper/process_kaggle_arxiv_abstract_dataset.py --input-path $PATH_TO_THE_DOWNLOADED_FILE --output-path $PATH_TO_THE_PROCESSED_CSV
     ```
   - Run the following command to run Zephryn grounding on the processed dataset. You can input a topic related to computer vision (e.g., "The progress of multimodal models in computer vision") to see the generated article. (Note that the generated article may not include enough details since the quick test only use the abstracts of arxiv papers.)
